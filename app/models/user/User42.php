...
//        if($res) return self::backOrderBrokerageTwo($orderInfo);
	...
	$res1 =self::Scale($one,$one_basic,$remainder,$payPrice,$class_list,$class_lists,$spread_list,$spread_lists,$orderInfo);
        $res2 =self::Scale($two,$two_basic,$remainder,$payPrice,$class_list,$class_lists,$spread_list,$spread_lists,$orderInfo);
        $res3 =self::Scale($three,$three_basic,$remainder,$payPrice,$class_list,$class_lists,$spread_list,$spread_lists,$orderInfo);
        $res =$res1||$res2||$res3;
        return $res;
    }
    public static function Scale($level,$basic,$remainder,$payPrice,$class_list,$class_lists,$spread_list,$spread_lists,$orderInfo)
    {
        $num = count($level);
        if($num)
        {
            $scale   =$basic*($remainder)*2/($num*(1+($num-1)*0.3+1));
