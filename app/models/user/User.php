...
//        if($res) return self::backOrderBrokerageTwo($orderInfo);
	...
	$res1 =self::Scale($one,$one_basic,$remainder,$payPrice,$class_list,$class_lists,$spread_list,$spread_lists,$orderInfo,$class,$one_first);
        $res2 =self::Scale($two,$two_basic,$remainder,$payPrice,$class_list,$class_lists,$spread_list,$spread_lists,$orderInfo,$class,$two_first);
        $res3 =self::Scale($three,$three_basic,$remainder,$payPrice,$class_list,$class_lists,$spread_list,$spread_lists,$orderInfo,$class,$three_first);
        $res =$res1||$res2||$res3;
        return $res;
    }
    public static function Scale($level,$basic,$remainder,$payPrice,$class_list,$class_lists,$spread_list,$spread_lists,$orderInfo,$class,$first)
    {
        $num = count($level);
        if($num)
        {
            $scales   =$basic*($remainder)*2/($num*(1+($num-1)*0.3+1));
            if($first!==''){
            if($class ==3){
                if($class_list[$first] ==2){
                    $first_basic = 0.4;//第一个
                    $first_scales = $first_basic*$remainder;
                    $basic =0;
                    if($num>1){
                        $scales   =$basic*($remainder)*2/(($num-1)*(1+($num-2)*0.3+1));
                    }
                }else if($class_list[$first] ==1){
                    $first_basic = 0.3;//第一个
                    $first_scales = $first_basic*$remainder;
                    $basic =0;
                    if($num>1){
            ...
            foreach($level as $k=>$v)
            {
             if($first === $v){
                  $scale =$first_scales;
		  $brokerageRatio =$scale;
             }else{
                  $scale =$scales;
		  $brokerageRatio =(($num-$k)*0.3+0.7)*$scale;
             }
            //TODO 返佣金额 = 毛利 / 二级返佣比例
            $brokeragePrice = bcmul($payPrice, $brokerageRatio, 2);
            //TODO 返佣金额小于等于0 直接返回不返佣金
            if ($brokeragePrice <= 0) return true;
            $userInfo = User::getUserInfo($spread_list[$v]);
            $balance = bcadd($userInfo['brokerage_price'], $brokeragePrice, 2);
            $orderuserInfo = User::getUserInfo($orderInfo['uid']);
            $mark =  $orderuserInfo['nickname'] .'成功消费' . floatval($orderInfo['pay_price']) . '元,'.$userInfo['nickname'] .'获得推广佣金' . floatval($brokeragePrice);
            self::beginTrans();
            ...
            $res2 = self::bcInc($spread_list[$v], 'brokerage_price',$brokeragePrice , 'uid');
            $res = $res1 && $res2;
            self::checkTrans($res);
            }
            return true;    
