...
class UserBill extends BaseModel
{
    ...
    public static function income($title,$uid,$category,$type,$number,$link_id = 0,$balance = 0,$mark = '',$class_list='',$spread_list='',$status=1)
    {
        $pm = 1;
        $add_time = time();
        return self::create(compact('title','uid','link_id','category','type','number','balance','mark','status','class_list','spread_list','pm','add_time'));
    }
