...
class User extends AuthController
{
    ...
    public function edit($uid)
    {
        ...
        $f[] = Form::radio('equal', '同级代理', $user->getData('equal'))->options([['value' => 1, 'label' => '是'], ['value' => 0, 'label' => '否']]);
	...
	$class_list=''; 
         if($data['is_promoter'])
         {
            $spread_infos = UserModel::get($user['spread_uid']);
          //上级推广者
         if($user['spread_uid']>0)
         {
             //有上级，
             $list = !empty($spread_infos['spread_list'])?($user['spread_uid'].','.$spread_infos['spread_list']):$user['spread_uid'];
             if($data['equal']){
                 //是否平级
                 //平级，class=上级的等级
                 //class_list=等级class+上级class_list
                $class = $spread_infos['class'];
                $class_list = !empty($spread_infos['class_list'])?($spread_infos['class'].','.$spread_infos['class_list']):$spread_infos['class'];
             }else{
                 //非平级，class=上级的等级+1
                 //class_list=等级class+上级class_list
                $class = $spread_infos['class']+1;
                $class_list = !empty($spread_infos['class_list'])?($spread_infos['class'].','.$spread_infos['class_list']):$spread_infos['class'];
             }
         }else{
             //无上级，表示1级，class=1，class_list='';
             $class = 1;
             $class_list ='';
         }
         }
	...
	$edit['equal'] = $data['equal'];
	$edit['spread_list'] = $list;//上级列表
	$edit['class'] = $class;//等级
        $edit['class_list'] = $class_list;//上级等级列表
        if ($edit) $res3 = UserModel::edit($edit, $uid);
        else $res3 = true;
        if ($res1 && $res2 && $res3) $res = true;
        else $res = false;
